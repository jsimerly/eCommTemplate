...
const Searchbar = () => {
    ...
    const StartDateInput = forwardRef(({ value, onClick }, ref) => (
        ...
        <div className={`bg-white cursor-pointer rounded-md mr-1 min-w-[140px] shadow focus-shadow-outline focus:outline-none placeholder-tertiaryTone-200 ${value == "" ? 'text-tertiaryTone-300': 'text-tertiary'} pl-10 p-2`} onClick={onClick} ref={ref} >
            {ref}
            {value == '' ? 'From' : value}
        </div>
        </label>
    ...
    <div className='flex-1 justify-start items-center max-w-3xl'>
        <div className='flex-1 sm:flex'>
            <label className="cursor-pointer relative text-tertiaryTone-200 focus-within:text-tertiary flex items-center flex-1">
                <svg 
                className="w-8 h-8 absolute transform "
                fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                ...
                <input className='cursor-pointer shadow focus:shadow-outline focus:outline-none flex-1 p-2 mr-1 rounded-md placeholder-tertiaryTone-200 text-tertiary pl-10' placeholder='Going to' type='text'/>
            </label>
